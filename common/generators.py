...
class PoseGenerator(Dataset):
    def __init__(self, poses_3d, poses_2d, actions, image_names=None, augment=False, kps_left=None, kps_right=None, joints_left=None, joints_right=None):
        ...
        pads = 27   # change frame
        poses_list = [[] for _ in range(len(poses_2d))]
        for i, poses in enumerate(poses_2d):
            for ppd in range(pads // 2):
                poses = np.insert(poses, 0, poses[0], axis=0)
                poses = np.insert(poses, -1, poses[-1], axis=0)
            for j in range(len(poses_2d[i])):
                temp = []
                for pad in range(pads):
                    temp.append(poses[j + pad])
                poses_list[i].append(temp)
            poses_list[i] = np.array(poses_list[i], dtype=np.float32)
        poses_2d = poses_list
        # end modify
