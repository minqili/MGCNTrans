...
from hp_common.utils import deterministic_random
from hp_common.camera import world_to_camera, normalize_screen_coordinates
from hp_common.generator_3dhp import ChunkedGenerator
...
class Fusion(data.Dataset):
    def __init__(self, opt, root_path, train=True, MAE=False):
        self.data_type = "h36m"
        self.train = train
        self.keypoints_name = "cpn_ft_h36m_dbb"
        self.root_path = root_path

        self.train_list = "S1,S5,S6,S7,S8".split(',')
        self.test_list = "S9,S11".split(',')
        ...
        if self.train:
            ...
            self.generator = ChunkedGenerator(opt.batch_size // self.stride, None, self.poses_train,
                                              ...
                                              joints_right=self.joints_right, out_all=1, MAE=MAE, train=True)
            ...
            self.generator = ChunkedGenerator(opt.batch_size // self.stride, None, self.poses_test,
    ...
    def __getitem__(self, index):
        ...
        if self.MAE:
            ...
            if self.train == False and self.test_aug:
                _, input_2D_aug, _, _, _ = self.generator.get_batch(seq_name, start_3d, end_3d, flip=True,
                                                                    reverse=reverse)
