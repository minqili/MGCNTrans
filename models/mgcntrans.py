...
from models.block.sem_graph_conv import SemGraphConv
...
class MGCNTrans(nn.Module):
    """
    The MGCNTrans model that predicts 3D poses from 2D poses.

    Arguments
    ---------
    num_joints_in: int
        Number of joints per pose. Default=17 for human36m.
    n_features: Tuple[int]
        Number of features per joint. Default=(54, 3) for temporal embedding->3D poses.
    n_layer: int
        Number of transformer encoder layers. Default=4.
    d_model: int
        Size of the hidden dimension per transformer encoder. Default=256.
    d_inner: int
        Size of the hidden dimension within the feed forward module inside the encoder transformer layers. Default=512.
    n_head: int
        Number of multi-head attention head. Default=8.
    ...
    def __init__(self, args, num_joints_in: int=17, n_features: Tuple[int]=(54, 3), n_layers: int=4, d_model: int=256,
                 d_inner: int=512, n_head: int=8, d_k: int=64, d_v: int=64, encoder_dropout: float=0.0, pred_dropout: float=0.2, 
        ...
        if self.spatial_encoding:
            self.position_enc = nn.Parameter(torch.zeros(1, num_joints_in, d_model))
        self.temporal_enc = nn.Parameter(torch.zeros(1, 34, 3))   # modify 添加的
        self.dropout = nn.Dropout(p=encoder_dropout)
        self.mlp = Mlp(in_features=128 * 51, hidden_features=256, out_features=d_model, drop=encoder_dropout)

        # Stacked encoders.
        self.layer_stack = nn.ModuleList([
            EncoderLayer(args, d_model, d_inner, n_head, d_k, d_v, dropout=encoder_dropout)
            for _ in range(n_layers)])
        self.layer_norm = nn.LayerNorm(d_model, eps=1e-6)
    ...
    def forward(self, src: torch.Tensor, image: torch.Tensor=None, return_attns: bool=False) -> Tuple[Union[List[torch.Tensor], torch.Tensor], torch.Tensor, Union[List[torch.Tensor], torch.Tensor], List[torch.Tensor]]:
        ...
        """
        b, f, j, c = src.shape  # Batch, Number of joints, Number of features per joint
        intermediate_list = []
        error_list = []
        enc_slf_attn_list = []

        # Expand dimensions.
        src = src.permute(0, 2, 1, 3).contiguous().view(b, j, -1)
        src = self.embedding_forward(src)
