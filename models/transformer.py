...
# modify
from models.block.graph_frames import Graph
from models.block.mlp_gcn import Mlp_gcn
...
class PositionwiseFeedForwardElse(nn.Module):
    ''' A two-feed-forward-layer module '''

    def __init__(self, d_in, d_hid, dropout=0.1):
        super().__init__()
        self.w_1 = nn.Linear(d_in, d_hid)  # position-wise
        self.w_2 = nn.Linear(d_hid, d_in // 2)  # position-wise
        self.layer_norm = nn.LayerNorm(d_in // 2, eps=1e-6)
        self.dropout = nn.Dropout(dropout)
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, args, d_model, d_inner, n_head, d_k, d_v, dropout=0.1):
        ...
        self.mlp_gcn = Mlp_gcn(args=args, depth=2 if args.h36mM else 3, embed_dim=256, channels_dim=512, tokens_dim=128, adj=self.A, length=17,
                               frames=1)
        if args.h36mM:
            self.mlp = nn.Parameter(torch.zeros(1, 17, 128))

    def forward(self, enc_input, slf_attn_mask=None):
        enc_output, enc_slf_attn = self.slf_attn(
            enc_input, enc_input, enc_input, mask=slf_attn_mask)

        enc_output_res = self.mlp_gcn(enc_output)
        enc_output = self.pos_ffn(enc_output_res)
        ...
        if intermediate:
            if mdn:
                ...
                self.intermediate_pred = nn.ModuleList(
                    [nn.Sequential(nn.Dropout(p=pred_dropout), nn.Linear(16 * d_model, 16 * 3)) for _ in
                     range(n_layers)])
...
class Transformer(nn.Module):
    ...
    def __init__(
            ...
            n_layers=6, n_head=8, dropout=0.2, d_k=64, d_v=64, intermediate=False, mdn=False, encoding=True,
            pred_dropout=0.0):
...
class LiftFormer(nn.Module):
    ...
    def __init__(self, num_joints_in, j_nfeatures,
        ...
        self.transformer_encoder = Transformer(t_nhid, d_word_vec=t_nhid, d_model=t_nhid, n_layers=t_nlayers,
                                               dropout=dropout, intermediate=intermediate, mdn=mdn and intermediate,
                                               encoding=not spatial_encoding, pred_dropout=pred_dropout)
    ...
    def forward(self, src, image_features=None):
        ...
        if self.intermediate:
            ...
            if self.mdn:
                ...
                out = [torch.cat([mu, sigma.unsqueeze(1), alpha.unsqueeze(1)], dim=1) for mu, sigma, alpha in
                       zip(intermediate_mu, intermediate_sigma, intermediate_alpha)]
            else:
