...
def train_worker(rank, addr, port, args):
    ...
    if args.evaluate and rank == 0:
        ...
        for i, action in enumerate(action_filter):
            ...
            error1, error2 = evaluate(valid_loader, model_pos, intermediate=args.intermediate, ttda=True,
    ...
    if rank == 0:
        poses_valid, poses_valid_2d, actions_valid, names_valid = fetch(subjects_test, dataset, keypoints,
                                                                        action_filter, stride, image_names=image_names)
        valid_loader = DataLoader(PoseGenerator(poses_valid, poses_valid_2d, actions_valid, names_valid),
...
def train(data_loader, model_pos, criterion, optimizer, lr_init, lr_now, step, decay, gamma, max_norm=True,
    ...
    start_point = [0, 1, 2, 0, 4, 5, 7, 7, 8, 9, 8, 11, 12, 8, 14, 15]
    end_point = [1, 2, 3, 4, 5, 6, 0, 8, 9, 10, 11, 12, 13, 14, 15, 16]
        ...
        if intermediate:
            ...
            for k in range(n_layers):
                ...
                if k == 3 & vis == True:
                    ...
                    ax1.set_title('Target-3D')
                    show3DposePair17(targets_3d[0].cpu().detach().numpy(), ax=ax1)
                    plt.savefig(os.path.join('./result', 'viz{}target.png'.format(str(i))), dpi=100)
                    plt.close(fig)

                    fig = plt.figure()
                    ax1 = fig.add_subplot(1, 1, 1, projection='3d')
                    ax1.set_title('Predict-3D')
                    show3DposePair17(outputs_3d[0].cpu().detach().numpy(), ax=ax1)
                    plt.savefig(os.path.join('./result', 'viz{}output.png'.format(str(i))), dpi=100)
                    plt.close(fig)

                    fig = plt.figure()
                    ax1 = fig.add_subplot(1, 1, 1, projection='3d')
                    ax1.set_title('Pair-3D')
                    show3DposePair16(targets_3d[0].cpu().detach().numpy(), outputs_3d[0].cpu().detach().numpy(), ax=ax1)
                    plt.savefig(os.path.join('./result', 'viz{}Pair.png'.format(str(i))), dpi=100)
                    plt.close(fig)
...
def show3DposePair16(realt3d, faket3d, ax, lcolor="#3498db", rcolor="#e74c3c", add_labels=True,
...
def show3DposePair17(realt3d, ax, lcolor="#3498db", rcolor="#e74c3c", add_labels=True,
                     gt=True, pred=False):  # blue, orange
    """
    Visualize a 3d skeleton pair
    ...
    for idx, vals in enumerate([realt3d]):
        # Make connection matrix
        for i in np.arange(len(I)):
            x, y, z = [np.array([vals[I[i], j], vals[J[i], j]]) for j in range(3)]
            if idx == 0:
